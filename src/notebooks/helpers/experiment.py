...
from helpers.computational_tools import (
    rename_coordinates,
    remesh,
    compute_isotropic_KE,
    compute_isotropic_cospectrum,
    compute_isotropic_PE,
    compute_KE_time_spectrum,
    mass_average,
    Lk_error,
    select_LatLon,
    diffx_uq,
...
class Experiment:
    ...
    def __init__(self, folder, key=""):
        ...
        self.recompute = (
            False  # default value of recomputing of cached on disk properties
        )
        ...
        for key in Experiment.get_list_of_main_properties():
            if compute:
                setattr(
                    ...
                    key,
                    operator(
                        self.__getattribute__(key), target.__getattribute__(key)
                    ).compute(),
                )
            else:
                setattr(
                    result,
                    key,
                    operator(self.__getattribute__(key), target.__getattribute__(key)),
                )
    ...
    def series(self):
        result = xr.open_dataset(
            os.path.join(self.folder, "ocean.stats.nc"), decode_times=False
        )
        return result
    ...
    def vert_grid(self):
        return xr.open_dataset(
            os.path.join(self.folder, "Vertical_coordinate.nc")
        ).rename({"Layer": "zl"})

    @cached_property
    def prog(self):
        result = xr.open_mfdataset(
            os.path.join(self.folder, "prog_*.nc"),
            decode_times=False,
            parallel=True,
            chunks={"Time": 5, "zl": 2},
        )
    ...
    def mom(self):
        result = xr.open_mfdataset(
            os.path.join(self.folder, "mom_*.nc"),
            decode_times=False,
            parallel=True,
            chunks={"Time": 5, "zl": 2},
        )
    ...
    def energy(self):
        result = xr.open_mfdataset(
            os.path.join(self.folder, "energy_*.nc"),
            decode_times=False,
            parallel=True,
            chunks={"Time": 5, "zl": 2},
        )
    ...
    def ave(self):
        result = xr.open_mfdataset(
            os.path.join(self.folder, "ave_*.nc"),
            decode_times=False,
            parallel=True,
            chunks={"Time": 5, "zl": 2},
        )
    ...
    def forcing(self):
        return xr.open_mfdataset(
            os.path.join(self.folder, "forcing_*.nc"), decode_times=False
        )
    ...
    def KE_spectrum_global(self):
        return self.KE_spectrum_global_series.sel(Time=self.Averaging_time).mean(
            dim="Time"
        )
    ...
    def transfer(self, fx, fy, **kw):
        return (
            compute_isotropic_cospectrum(
                self.u, self.v, fx, fy, self.param.dxT, self.param.dyT, **kw
            )
            .sel(Time=self.Averaging_time)
            .mean(dim="Time")
        )

    def power(self, fx, fy, **kw):
        return 2 * compute_isotropic_KE(
            fx, fy, self.param.dxT, self.param.dyT, **kw
        ).sel(Time=self.Averaging_time).mean(dim="Time")
    ...
    def MKE_spectrum(self):
        return compute_isotropic_KE(
            self.u_mean, self.v_mean, self.param.dxT, self.param.dyT
        )
    ...
    def PE_spectrum(self):
        H0 = 1000.0  # 1000 is reference depth (see series.H0.isel(Interface=1))
        hint = (
            self.e.isel(zi=1) + H0
        )  # https://github.com/NOAA-GFDL/MOM6/blob/dev/gfdl/src/diagnostics/MOM_sum_output.F90#L655
        mask = (
            self.h.isel(zl=1) > 1e-9
        )  # mask of wet points. Boundaries have values 1e-10; https://github.com/NOAA-GFDL/MOM6/blob/dev/gfdl/src/diagnostics/MOM_sum_output.F90#L656
        hint = hint * mask
        return (
            compute_isotropic_PE(hint, self.param.dxT, self.param.dyT)
            .sel(Time=self.Averaging_time)
            .mean(dim="Time")
        )
    ...
    def EKE_spectrum_direct(self):
        ...
        return compute_isotropic_KE(
            u_eddy, v_eddy, self.param.dxT, self.param.dyT
        ).mean(dim="Time")
    ...
    def logKEz(self):
        return np.log10(
            0.5
            * (
                self.h * (remesh(self.u**2, self.e) + remesh(self.v**2, self.e))
            ).sum("zl")
        )
    ...
    def EKE(self):
        ...
        return np.maximum(
            self.KE.sel(Time=self.Averaging_time).mean(dim="Time") - MKE, 0
        )
    ...
    def EKE_joul(self):
        return (
            self.KE_joul_series.sel(Time=self.Averaging_time).mean(dim="Time")
            - self.MKE_joul
        )
    ...
    def PE_joul(self, e):
        ...
        hbot = np.maximum(
            self.e.isel(zi=2, Time=0) + 1000, 0
        )  # Time-independent bottom profile, where there is no fluid at rest; https://github.com/NOAA-GFDL/MOM6/blob/dev/gfdl/src/diagnostics/MOM_sum_output.F90#L656
        return (
            0.5
            * self.vert_grid.R.isel(zl=1)
            * self.vert_grid.g.isel(zl=1)
            * ((hint**2 - hbot**2) * self.param.dxT * self.param.dyT).sum(
                dim=("xh", "yh")
            )
        )  # https://github.com/NOAA-GFDL/MOM6/blob/dev/gfdl/src/diagnostics/MOM_sum_output.F90#L657

    def PE_ssh(self, e):
        hint = e.isel(zi=0)
        return (
            0.5
            * self.vert_grid.R.isel(zl=0)
            * self.vert_grid.g.isel(zl=0)
            * (hint**2 * self.param.dxT * self.param.dyT).sum(dim=("xh", "yh"))
        )
    ...
    def EPE_joul(self):
        return (
            self.PE_joul_series.sel(Time=self.Averaging_time).mean(dim="Time")
            - self.MPE_joul
        )
    ...
    def EPE_ssh(self):
        return (
            self.PE_ssh_series.sel(Time=self.Averaging_time).mean(dim="Time")
            - self.MPE_ssh
        )
    ...
    def meridional_profile(self, field, h=None, npoints=20):
        ...
        return (
            (remesh(field, h) * h * self.param.dxT)
            .sum(dim=("zl", "xh"))
            .sel(Time=self.Averaging_time)
            .mean(dim="Time")
            .coarsen({"yh": nfactor})
    ...
    def SGS_flux_profile(self):
        ...
        return (
            self.meridional_profile(S11),
            self.meridional_profile(S12),
            self.meridional_profile(S22),
        )
    ...
    def SGS_profile(self, npoints=20):
        ...
        fy = (
            (SGSy * h_v * self.param.dxCv)
            .sum(dim=("xh"))
            .sel(Time=self.Averaging_time)
            .mean(dim="Time")
            .coarsen({"yq": nfactor}, boundary="trim")
            .mean()
            .compute()
        )
        return fx, fy
    ...
    def Smagorinsky_flux_profile(self, **kw_Smagorinsky):
        (
            ...
            S12,
            S22,
        ) = self.Smagorinsky(**kw_Smagorinsky)
        return (
            self.meridional_profile(S11),
            self.meridional_profile(S12),
            self.meridional_profile(S22),
    ...
    def KE_Arakawa(self):
        ...
        return (
            0.5
            * (remesh(areaCu * self.u**2, areaT) + remesh(areaCv * self.v**2, areaT))
            / areaT
        )
    ...
    def gradKE(self):
        ...
        """
        KE = self.KE_Arakawa
        IdxCu = 1.0 / self.param.dxCu
        IdyCv = 1.0 / self.param.dyCv
    ...
    def relative_vorticity(self):
        ...
        IareaBu = 1.0 / (self.param.dxBu * self.param.dyBu)
    ...
    def subgrid_momentum_flux(self):
        ...
        S_12 = -remesh(remesh(u, h) * remesh(v, h), RVc) + remesh(uc, RVc) * remesh(
            vc, RVc
        )
    ...
    def hq(self):
        ...
        return (2.0 * (h2uq * h2vq)) / (
            (h2uq + h2vq)
            * (2.0 * remesh(self.h_u, self.RV) + 2.0 * remesh(self.h_v, self.RV))
        )
    ...
    def divergence(self, S_11, S_12, S_22, h=True):
        ...
        IareaCu = 1.0 / (self.param.dxCu * self.param.dyCu)
        IareaCv = 1.0 / (self.param.dxCv * self.param.dyCv)
        ...
        fx = (
            IdyCu * diffx_uq(dy2h * S_11, IdyCu) + IdxCu * diffy_tu(dx2q * S_12, IdxCu)
        ) * IareaCu
        ...
        fy = (
            IdyCv * diffx_tv(dy2q * S_12, IdyCv) + IdxCv * diffy_vq(dx2h * S_22, IdxCv)
        ) * IareaCv
    ...
    def ZB_offline(
        ...
        **kw,
        ...
        amp = xr.DataArray(
            [amplitude, amp_bottom if amp_bottom > -0.5 else amplitude], dims=["zl"]
        )
        ...
        if ZB_type == 0:
            sum_sq = 0.5 * (
                remesh(vort_xy, self.h) ** 2 + remesh(sh_xy, self.h) ** 2 + sh_xx**2
            )
        elif ZB_type == 1:
            sum_sq = 0.0
    ...
    def ZB_offline_cartesian(self, amplitude=1.0 / 24):
        D = self.sh_xy()
        ...
        fx = (
            diffx_uq(S_11, self.u) / self.param.dxCu
            + diffy_tu(S_12, self.u) / self.param.dyCu
        )
        fy = (
            diffx_tv(S_12, self.v) / self.param.dxCv
            + diffy_vq(S_22, self.v) / self.param.dyCv
        )
    ...
    def Smagorinsky(self, Cs=0.03):
        ...
        # Del2u = Laplace(u)
        Del2u, Del2v = self.divergence(
            self.sh_xx(), self.sh_xy(), -self.sh_xx(), h=False
        )
    ...
    def Reynolds(self, nwidth=1, nselect=1, Cr=10):
        ...
        S_12 = remesh(bar(ud), self.RV) * remesh(bar(vd), self.RV) - bar(
            remesh(ud, self.RV) * remesh(vd, self.RV)
        )
    ...
    def SSM(self, nwidth=1, nselect=1, C=1):
        ...
        S_12 = remesh(bar(ub), self.RV) * remesh(bar(vb), self.RV) - bar(
            remesh(ub, self.RV) * remesh(vb, self.RV)
        )
