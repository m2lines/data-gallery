...
class netcdf_property:
    ...
    def __get__(self, instance, owner):
        ...
        """
        if instance is None:
            return self  # see https://gist.github.com/asross/952fa456f8bcd07abf684cc515d49030
        ...
        filename = os.path.join(
            ...
            "-".join(instance.folder.split("/")[4:-2])
            + "-"
            + instance.key
            + "-"
            + funcname
        ...
        if os.path.exists(filename):
            ...
            try:
                ncfile = xr.open_dataset(
                    filename, decode_times=False, chunks={"Time": 1, "zl": 1}
                )
            except:
                ncfile = xr.open_dataset(
                    filename, decode_times=False
...
def free_of_NaNs_and_zeros(value):
    ...
    if len(value.dims) <= 1:
        if value.notnull().sum() == value.size:  # every scalar value is not NaN
            if (value == 0.0).sum() != value.size:  # every scalar value is not 0.
                return True
            else:
                False
        else:
