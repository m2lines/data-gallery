...
def grid_spacing(param):
    IdxCv = 1.0 / param.dxCv.values
    IdyCu = 1.0 / param.dyCu.values
    dyCv = param.dyCv.values
    dxCu = param.dxCu.values
    IareaBu = 1.0 / (param.dxBu * param.dyBu).values
...
def compute_diag(psi, IdxCv, IdyCu, dyCv, dxCu, IareaBu):
    ...
    for j in range(1, Ny - 1):
        for i in range(1, Nx - 1):
            p = 0 * psi
            p[j, i] = 1.0
            D[j - 1, i - 1] = laplace(p, IdxCv, IdyCu, dyCv, dxCu, IareaBu)[
                j - 1, i - 1
            ]
    return D
...
def jacobi_iteration(RV, param, eps=1e-6):
    ...
    for i in range(1000000000):
        ...
        rel_residual = np.sqrt(((RV - laplace(psi, *grid)) ** 2).mean()) / np.sqrt(
            ((RV) ** 2).mean()
        )
        if rel_residual < eps:
            break
...
def optimal_amplitude(ZBx, ZBy, Smagx, Smagy, SGSx, SGSy, u, v, amp_Eng):
    ...
    MSE_Eng = (
        fMSE(amp_Eng * ZBx + Smagx, SGSx) + fMSE(amp_Eng * ZBy + Smagy, SGSy)
    ) / 2

    corr = (
        xr.corr(ZBx.chunk({"zl": 1, "Time": 1}), SGSx, dim=dim_u)
        + xr.corr(ZBy.chunk({"zl": 1, "Time": 1}), SGSy, dim=dim_v)
    ) / 2
...
def filter_apply(q):
    ...
    def sel(i, j):
        qsel = q.isel(
            {
                x.name: slice(1 + i, -1 + i if i < 1 else None),
                y.name: slice(1 + j, -1 + j if j < 1 else None),
            }
        )
    ...
    wside = 1.0 / 8.0
    wcorner = 1.0 / 16.0
    wcenter = 1.0 - (wside * 4.0 + wcorner * 4.0)
...
def select_LatLon(array, Lat=(35, 45), Lon=(5, 15)):
    ...
    return array.sel({x.name: slice(Lon[0], Lon[1]), y.name: slice(Lat[0], Lat[1])})
...
def remesh(input, target, fillna=True):
    ...
    if ratiox > 1 or ratioy > 1:
        ...
        result = result.coarsen(
            {x_input.name: ratiox, y_input.name: ratioy}, boundary="pad"
        ).mean()
...
def compute_isotropic_KE(
    ...
    Eu = xrft.isotropic_power_spectrum(
        u,
        dim=("xh", "yh"),
        window=window,
        nfactor=nfactor,
    ...
    Ev = xrft.isotropic_power_spectrum(
        v,
        dim=("xh", "yh"),
        window=window,
        nfactor=nfactor,
    ...
    E["freq_r"] = (
        E["freq_r"] * 2 * np.pi
    )  # because library returns frequencies, but not wavenumbers
...
def compute_isotropic_cospectrum(
    u_in,
    v_in,
    fu_in,
    fv_in,
    ...
    Eu = xrft.isotropic_cross_spectrum(
        u,
        fu,
        dim=("xh", "yh"),
        window=window,
    ...
    Ev = xrft.isotropic_cross_spectrum(
        v,
        fv,
        dim=("xh", "yh"),
        window=window,
...
def compute_isotropic_PE(
    ...
    E = xrft.isotropic_power_spectrum(
        hint,
        dim=("xh", "yh"),
        window=window,
        nfactor=nfactor,
...
def mass_average(KE, h, dx, dy):
    return (KE * h * dx * dy).mean(dim=("xh", "yh")) / (h * dx * dy).mean(
        dim=("xh", "yh")
    )
...
def Lk_error(input, target, normalize=False, k=2):
    ...
    """
    # Check dimensions
    if sorted(input.dims) != sorted(target.dims) or sorted(input.shape) != sorted(
        target.shape
    ):
        import sys

        sys.exit(
            f"Dimensions disagree: {sorted(input.dims)} {sorted(target.dims)} {sorted(input.shape)} {sorted(target.shape)}"
        )
    ...
    def lk_norm(x, k):
        ...
        """
        if k > 0:
            return ((np.abs(x) ** k).mean(dim=average_dims)) ** (1.0 / k)
        elif k == -1:
