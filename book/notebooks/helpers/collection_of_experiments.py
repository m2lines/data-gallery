...
class CollectionOfExperiments:
    ...
    def remesh(
        self, input, target, exp=None, name=None, compute=False, operator=remesh
    ):
        """
        ...
        result = self[input].remesh(
            self[target], exp, compute, operator
        )  # call experiment method
    ...
    def plot_KE_spectrum(self, exps, key="EKE_spectrum", labels=None):
        ...
        for j, exp in enumerate(exps):
            ...
            p.extend(
                ax[0].loglog(
                    k, KE_upper, lw=3, label=labels[j], color=color.get(exp, None)
                )
            )
            ...
            p.extend(
                ax[1].loglog(
                    k, KE_lower, lw=3, label=labels[j], color=color.get(exp, None)
                )
            )
    ...
    def plot_transfer(self, exp, target="R64_R4", callback=True):
        ...
        for zl in range(2):
            ...
            ax2 = plt.gca().secondary_xaxis(
                "top", functions=(lambda x: x / kmax, lambda x: x * kmax)
            )
    ...
    def plot_ssh_std(self, exps, labels=None, target="R64_R2"):
        ...
        for ifig, exp in enumerate(exps):
            ...
            if exp != exps[-1]:
                RMSE = Lk_error(
                    ssh, remesh(self[exps[-1]].ssh_std, self[target].ssh_std)
                )[0]
                plt.text(
                    9, 31, "RMSE=" + str(round(RMSE, 3)) + "$m$", fontsize=14, color="w"
                )
    ...
    def plot_KE_PE_simpler(
        ...
        plt.axhspan(
            MKE[-2] + EKE[-2], MKE[-1] + EKE[-1], color=color[1], alpha=0.1, lw=0
        )
        plt.yticks([0, 5, 10, 15, 20, 25, 30])
    ...
    def plot_domain(self, axes=None):
        def plot(axes, xangle, yangle, topography, interface, free_surface, taux):
            ...
            for x, y, z, v in zip(xx, yy, zz, vector):
                ...
                axes.add_collection3d(
                    Poly3DCollection(arrow, color="tab:blue", zorder=10)
                )
        ...
        topography = (
            self["R2"].e.isel(zi=2, Time=-1).coarsen(xh=2, yh=2, boundary="trim").mean()
        )
        interface = xr.where(topography > -1000, np.nan, -1000)
        free_surface = self["R64_R4"].ssh_mean
        taux = (
            self["R2"].forcing.taux.isel(Time=-1).coarsen(yh=2, boundary="trim").mean()
        )
